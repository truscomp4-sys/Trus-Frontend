...
export const handleDocumentDownload = (url: string, filename?: string) => {
  ...
  const downloadUrl = `${apiBase}/upload/download?url=${encodeURIComponent(url)}${filename ? `&filename=${encodeURIComponent(filename)}` : ''}`;
