...
const ServiceDetail = ({ id }: { id: string }) => {
    ...
    if (error || !service) { if (typeof window !== "undefined") window.location.replace("/services"); return null; }
...
const ActionCard = ({ doodleType }: any) => {
    return (
        <div className="bg-card rounded-2xl p-6 border border-border shadow-lg relative overflow-hidden">
            ...
            <Link href="/contact" className="w-full btn-primary py-3 rounded-lg flex items-center justify-center gap-2 font-bold shadow-md hover:shadow-lg transition-all">
